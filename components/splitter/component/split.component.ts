...
import { SPLIT_DEFAULT_OPTIONS } from '../split-config.token'
  ...
  template: ` <ng-content></ng-content>
    ...
    >
      <div
        ...
        *ngIf="last === false"
        #gutterEls
        class="gmtre-split-gutter"
        [class.gmtre-dragged]="draggedGutterNum === index + 1"
      ...
      >
        <ng-container *ngIf="customGutter?.template; else defaultGutterTpl">
          <ng-container *gmtreSplitGutterDynamicInjector="index + 1; let injector">
            <ng-container
              *ngTemplateOutlet="
        ...
        <ng-template #defaultGutterTpl>
          <div class="gmtre-split-gutter-icon"></div>
        </ng-template>
      </div>
    </ng-template>`,
...
export class SplitComponent implements AfterViewInit, OnDestroy {
  ...
  @Input() set direction(v: ISplitDirection) {
    ...
    this.renderer.addClass(this.elRef.nativeElement, `gmtre-${this._direction}`)
    this.renderer.removeClass(
      this.elRef.nativeElement,
      `gmtre-${this._direction === 'vertical' ? 'horizontal' : 'vertical'}`,
    )
  ...
  @Input() set unit(v: ISplitUnit) {
    ...
    this.renderer.addClass(this.elRef.nativeElement, `gmtre-${this._unit}`)
    this.renderer.removeClass(this.elRef.nativeElement, `gmtre-${this._unit === 'pixel' ? 'percent' : 'pixel'}`)
  ...
  @Input() set useTransition(v: boolean | `${boolean}`) {
    ...
    if (this._useTransition) {
      this.renderer.addClass(this.elRef.nativeElement, 'gmtre-transition')
    } else {
      this.renderer.removeClass(this.elRef.nativeElement, 'gmtre-transition')
    }
  }
  ...
  @Input() set disabled(v: boolean | `${boolean}`) {
    ...
    if (this._disabled) {
      this.renderer.addClass(this.elRef.nativeElement, 'gmtre-disabled')
    } else {
      this.renderer.removeClass(this.elRef.nativeElement, 'gmtre-disabled')
    }
  }
  ...
  constructor(
    ...
    @Optional() @Inject(SPLIT_DEFAULT_OPTIONS) globalConfig: IDefaultOptions,
  ) {
  ...
  ngAfterViewInit() {
    this.ngZone.runOutsideAngular(() => {
      ...
      setTimeout(() => this.renderer.addClass(this.elRef.nativeElement, 'gmtre-init'))
    })
  }
    ...
    if (this.isWaitingInitialMove) {
      if (this.startPoint.x !== this.endPoint.x || this.startPoint.y !== this.endPoint.y) {
        this.ngZone.run(() => {
          ...
          this.renderer.addClass(this.elRef.nativeElement, 'gmtre-dragging')
  ...
  private stopDragging(event?: Event): void {
    ...
    this.renderer.removeClass(this.elRef.nativeElement, 'gmtre-dragging')
  ...
  collapseArea(comp: SplitAreaDirective, newSize: number, gutter: 'left' | 'right'): void {
    ...
    if (gtr) {
      this.renderer.addClass(gtr.nativeElement, 'gmtre-split-gutter-collapsed')
    }
  ...
  expandArea(comp: SplitAreaDirective): void {
    ...
    if (gtr) {
      this.renderer.removeClass(gtr.nativeElement, 'gmtre-split-gutter-collapsed')
    }
    this.updateArea(comp, false, false)
  }
