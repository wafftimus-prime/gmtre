 ...
 */
@Directive({
  selector: '[gmtreScrollbar]',
  exportAs: 'gmtreScrollbar',
  standalone: true,
})
export class GmtreScrollbarDirective implements OnChanges, OnInit, OnDestroy {
  /* eslint-disable @typescript-eslint/naming-convention */
  static ngAcceptInputType_gmtreScrollbar: BooleanInput;
  /* eslint-enable @typescript-eslint/naming-convention */

  @Input() gmtreScrollbar: boolean = true;
  @Input() gmtreScrollbarOptions!: PerfectScrollbar.Options|any;

  private _animation!: number|any;
  private _options!: PerfectScrollbar.Options;
  private _ps!: PerfectScrollbar|any;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    // Enabled
    if ('gmtreScrollbar' in changes) {
      // Interpret empty string as 'true'
      this.gmtreScrollbar = coerceBooleanProperty(
        changes['gmtreScrollbar'].currentValue
      );

      // If enabled, init the directive
      if (this.gmtreScrollbar) {
        this._init();
      }
    ...
    // Scrollbar options
    if ('gmtreScrollbarOptions' in changes) {
      // Merge the options
      this._options = merge(
        {},
        this._options,
        changes['gmtreScrollbarOptions'].currentValue
      );
