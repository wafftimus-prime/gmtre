...
export class MailboxMockApi {
  ...
  registerHandlers(): void {
    ...
    // @ Folders - GET
    // -----------------------------------------------------------------------------------------------------
    this._gmtreMockApiService.onGet('api/apps/mailbox/folders').reply(() => {
      let count = 0;
    ...
    this._gmtreMockApiService
      ...
      .reply(({ request }) => {
        ...
        // Generate an id
        label.id = GmtreMockApiUtils.guid();
      ...
      .reply(({ request }) => {
        ...
        mails = mails.filter((mail) => {
          if (byFolder) {
            return (
              mail.folder ===
              this._folders.find((folder) => folder.slug === byFolder).id
            );
          }
          ...
          if (byLabel) {
            return mail.labels.includes(
              this._labels.find((label) => label.slug === byLabel).id
            );
          }
        });
