  ...
  registerHandlers(): void {
    ...
    this._gmtreMockApiService
      ...
      .reply(({ request }) => {
        ...
        boards = boards.map((board:any) => ({
          ...board,
          members: board.members.map((boardMember:any) =>
            this._members.find((member:any) => boardMember === member.id)
          ),
        }));
    ...
    this._gmtreMockApiService
      ...
      .reply(({ request }) => {
        ...
        // Find the board
        const board = this._boards.find((item:any) => item.id === id);

        // Attach the board lists
        board.lists = this._lists
          .filter((item:any) => item.boardId === id)
          .sort((a, b) => a.position - b.position);

        // Grab all cards that belong to this board and attach labels to them
        let cards = this._cards.filter((item:any) => item.boardId === id);
        cards = cards.map((card) => ({
          ...card,
          labels: card.labels.map((cardLabelId:any) =>
            this._labels.find((label) => label.id === cardLabelId)
          ),
        }));

        // Attach the board cards into corresponding lists
        board.lists.forEach((list:any, index:any, array:any) => {
          array[index].cards = cards
            .filter((item:any) => item.boardId === id && item.listId === list.id)
            .sort((a, b) => a.position - b.position);
        });

        // Attach the board labels
        board.labels = this._labels.filter((item:any) => item.boardId === id);
    ...
    this._gmtreMockApiService
      ...
      .reply(({ request }) => {
        ...
        // Generate a new GUID
        newList.id = GmtreMockApiUtils.guid();
    ...
    this._gmtreMockApiService
      ...
      .reply(({ request }) => {
        ...
        // Generate a new GUID
        newCard.id = GmtreMockApiUtils.guid();
    ...
    this._gmtreMockApiService
      ...
      .reply(({ request }) => {
        ...
        card.labels = card.labels.map((itemLabel:any) => itemLabel.id);
        ...
        updatedCard.labels = updatedCard.labels.map((cardLabelId:any) =>
          this._labels.find((label) => label.id === cardLabelId)
        );
    ...
    this._gmtreMockApiService
      ...
      .reply(({ request }) => {
        ...
        cards.forEach((item:any) => {
          // Find the card
          const index = this._cards.findIndex((card:any) => item.id === card.id);
    ...
    this._gmtreMockApiService
      ...
      .reply(({ request }) => {
        ...
        // Generate a new GUID
        newLabel.id = GmtreMockApiUtils.guid();
