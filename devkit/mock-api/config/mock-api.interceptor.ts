...
import { GMTRE_MOCK_API_DEFAULT_DELAY } from './mock-api.constants';
import { GmtreMockApiService } from './mock-api.service';
...
export const mockApiInterceptor = (
  ...
  const defaultDelay = inject(GMTRE_MOCK_API_DEFAULT_DELAY);
  const gmtreMockApiService = inject(GmtreMockApiService);

  // Try to get the request handler
  const { handler, urlParams } = gmtreMockApiService.findHandler(
