...
import { UserStore, GmtrePlatformConfigService } from '@gmtre-devkit';
...
export class AuthService {
  ...
  signOut(): Observable<any> {
    signOut().then((r) => (this._authenticated = false));
  ...
  async currentSession() {
    try {
      ...
      this.config.entrypoint = session;
      return session;
    } catch (err) {
      return undefined;
    }
  ...
  async currentAuthenticatedUser(): Promise<boolean> {
    try {
      ...
      userAttributes['id'] = userAttributes.sub;
      // this._userService.update(userAttributes);
      await this._userStore.loadUserAuth(userAttributes);
