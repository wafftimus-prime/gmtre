    ...
    }) =>
    {
        ...
        addComponents(
            _.fromPairs(_.map(options.themes, (theme, themeName) => [
                ...
                ))), (value, key) => [[`--gmtre-${e(key)}`, value], [`--gmtre-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
            ])),
        );
        ...
        const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
        {
            ...
            const background = theme(`gmtre.customProps.background.${colorScheme}`);
            const foreground = theme(`gmtre.customProps.foreground.${colorScheme}`);
            ...
            return {
                [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                    ...
                    ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--gmtre-${e(key)}`, value], [`--gmtre-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                    ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--gmtre-${e(key)}`, value], [`--gmtre-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                },
            };
        });
            ...
            theme: {
                extend: {
                    ...
                    colors: _.fromPairs(_.flatten(_.map(_.keys(flattenColorPalette(normalizeTheme(options.themes.default))), (name) => [
                        [name, `rgba(var(--gmtre-${name}-rgb), <alpha-value>)`],
                        [`on-${name}`, `rgba(var(--gmtre-on-${name}-rgb), <alpha-value>)`],
                    ]))),
                },
                gmtre  : {
                    customProps: {
